...
class Keys:
	...
	KEY_LEFTMETA        = 0xDB # Left Windows/Option key 
	KEY_RIGHTMETA       = 0xDC # Right Windows/Option key 
	...
	def getname(cls, n):
		for key in dir(cls):
			if not key.startswith('KEY_'): continue
			if getattr(cls, key) == n:
				return key
		else:
			raise IndexError
